...
@interface RecordEngine ()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureAudioDataOutputSampleBufferDelegate, CAAnimationDelegate>
{
...
@property (strong, nonatomic) RecordEncoder           *recordEncoder;//录制编码
...
- (instancetype)initWithMaxTime:(CGFloat)aMaxTime {
    self = [super init];
    if (self) {
        self.maxRecordTime = aMaxTime;
    }
    return self;
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    @synchronized(self) {
        ...
        if ((self.recordEncoder == nil) && !isVideo) {
            ...
            self.recordEncoder = [RecordEncoder encoderForPath:self.videoPath Height:_cy width:_cx channels:_channels samples:_samplerate];
        }
        //判断是否中断录制过
        if (self.discont) {
            if (isVideo) {
